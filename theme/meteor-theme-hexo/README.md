...
2. _(less ideal)_ Cloning this repository into the doc deployment's `theme/meteor` directory and referring to the `theme` in that deployment's `_config.yml` as just `meteor`.  This will require you to update the theme (or at least its submodule) manually!
